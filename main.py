...
class OtherFilter(telebot.custom_filters.AdvancedCustomFilter):
    key='add_domains'
    @staticmethod
    def check(message, text):
	    print( f'this is what im checking {message} and the text {text[0]}')
	    print(True if message.text in 'ADD Domains' else False)
	    return message.text in 'ADD Domains'
